...
public class ApiTest extends TestBase {
     ...
     * 所有公共header，会在发送请求的时候添加到http header上
     */
    private static Map<String, String> commonKeys = new HashMap<String, String>();
    /**
    ...
    public void init(@Optional("api-config.xml") String envName) throws Exception {
        ...
        commonHeaders = apiConfig.getHeaders();
    ...
    public void apiTest(ApiDataBean apiDataBean) throws Exception {
        ...
        HashMap querys = null;
        if (apiParam != "") {
            querys = JSON.parseObject(apiParam, HashMap.class);
        }
        String url = apiDataBean.getUrl();
        HttpUriRequest method = parseHttpRequest(url, apiDataBean.getMethod(), apiParam);
        String responseData;
        try {
            HttpResponse response = HttpUtils.doGet(rootUrl, apiDataBean.getUrl(), apiDataBean.getMethod(), commonHeaders, querys);
